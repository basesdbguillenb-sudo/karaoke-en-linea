...
// AISLAMIENTO DOM CON API OFICIAL: Solución definitiva al salto automático
const ReproductorNativo = memo(({ videoId, onEnd }: { videoId: string, onEnd: () => void }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const onEndRef = useRef(onEnd);

  // Mantenemos la referencia de onEnd siempre actualizada sin re-renderizar
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Creamos un div con ID único que la API de YouTube reemplazará
    const playerId = `yt-player-${Math.random().toString(36).substring(2, 11)}`;
    ...
    // Función constructora oficial del reproductor
    const initPlayer = () => {
      player = new (window as any).YT.Player(playerId, {
        ...
        events: {
          onStateChange: (event: any) => {
            // El estado 0 es universalmente reconocido por la API de YouTube como "ENDED" (Terminado)
            if (event.data === 0) {
              onEndRef.current();
            }
          }
    ...
    // Si la API ya está cargada en memoria, inicializamos directo
    if ((window as any).YT && (window as any).YT.Player) {
      initPlayer();
    } else {
      // Si no, preparamos un escuchador para cuando termine de cargar
      const listener = () => initPlayer();
      window.addEventListener('YouTubeAPIReady', listener);
      
      // Declaramos la función global que YouTube busca al inyectar su script
      if (!(window as any).onYouTubeIframeAPIReady) {
        (window as any).onYouTubeIframeAPIReady = () => {
          window.dispatchEvent(new Event('YouTubeAPIReady'));
        };
        
        // Inyectamos el script oficial de YouTube Iframe API
      ...
      // Limpieza en caso de que se desmonte el componente antes de cargar
      return () => {
         window.removeEventListener('YouTubeAPIReady', listener);
  ...
  }, [videoId]); // El useEffect depende estrictamente del videoId para evitar parpadeos

  // MAXIMIZACIÓN TOTAL ASEGURADA: fixed, top-0, left-0, w-screen, h-screen, z-[9999]
  return (
    <div className="fixed top-0 left-0 w-screen h-screen z-[9999] bg-black">
        <div ref={containerRef} className="w-full h-full relative" />
        <button 
          onClick={() => onEndRef.current()}
          className="absolute bottom-10 right-10 bg-red-600 hover:bg-red-500 text-white font-bold py-4 px-8 rounded-full shadow-[0_0_20px_rgba(220,38,38,0.9)] transition-all hover:scale-110 z-[10000] text-xl flex items-center gap-2 backdrop-blur-sm"
        >
          ⏭ Saltar Pista
        </button>
    </div>
...
export default function TvScreen() {
  ...
  useEffect(() => {
    ...
    // Escucha en tiempo real: Si alguien añade una canción desde su celular, se actualiza al instante
  ...
  // useCallback asegura que el ReproductorNativo no se recargue innesesariamente
  ...
  const iniciarPantallaCompleta = () => {
    ...
    // Forzamos el navegador a pantalla completa
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-white font-sans overflow-hidden" onDrop={handleDrop} onDragOver={handleDragOver}>
      ...
      {/* EL REPRODUCTOR MAXIMIZADO (Solo se muestra en fase 'cantando') */}
      ...
      {/* SECCIÓN PRINCIPAL (FONDO Y ESTADOS) */}
